...
export default async function DashboardLayout({ children }: { children: React.ReactNode }) {
  ...
  const items: NavItem[] = [{ href: "/dashboard", label: "Resumen", grupo: "entrenamiento" }];
  ...
  items.push(
    { href: "/dashboard/routines", label: "Rutinas", grupo: "entrenamiento" },
    { href: "/dashboard/exercises", label: "Ejercicios", grupo: "entrenamiento" },
  );

  if (org.tipo === "gimnasio") {
    items.push({ href: "/dashboard/sedes", label: "Sedes", grupo: "gestion" });
  }
  if (org.tipo !== "individual" && esDueno(org.rol)) {
    items.push({ href: "/dashboard/equipo", label: "Equipo", grupo: "gestion" });
  }

  if (org.tipo !== "individual" && puedeVerPlata(org.rol)) {
    items.push(
      { href: "/dashboard/planes", label: "Planes", grupo: "negocio" },
      { href: "/dashboard/finanzas", label: "Finanzas", grupo: "negocio" },
      { href: "/dashboard/productos", label: "Productos", grupo: "negocio" },
    );
  }

  items.push({ href: "/dashboard/settings", label: "Configuración", grupo: "gestion" });
